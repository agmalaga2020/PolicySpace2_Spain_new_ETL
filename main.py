...
logging.basicConfig(level=logging.INFO) # Default for most loggers
logging.getLogger('generator').setLevel(logging.DEBUG) # Set generator logger to DEBUG
logging.getLogger('world.firms').setLevel(logging.DEBUG) # Also for firms if needed
logging.getLogger('world.population').setLevel(logging.DEBUG) # And population
...
def single_run(params, path):
    ...
    if conf.RUN['PRINT_STATISTICS_AND_RESULTS_DURING_PROCESS']:
        # Configure basicConfig first, then set specific loggers to DEBUG if needed
        logging.basicConfig(level=logging.INFO) 
        logging.getLogger('generator').setLevel(logging.DEBUG)
        logging.getLogger('world.firms').setLevel(logging.DEBUG)
        logging.getLogger('world.population').setLevel(logging.DEBUG)
        # Ensure main logger also respects this if it was DEBUG before
        # Or, if we want main to be INFO but others DEBUG, this is fine.
    else: # If not printing stats, maybe we want full debug for everything?
        logging.basicConfig(level=logging.DEBUG) # Or keep INFO and set specifics
