...
OUTPUT_DATA_SPEC = {
    ...
    'regional': {
        ...
        'columns': ['month', 'mun_id', 'commuting', 'pop', 'gdp_region',
                    ...
                    'qli_index', 'gdp_percapita', 'treasure', 'equally', 'locally', 'pie', # Changed 'fpm' to 'pie'
                    'licenses']
    }
}
...
class Output:
    ...
    def __init__(self, sim, output_path):
        ...
        # For Spanish adaptation, use municipality codes instead of ACP codes
        municipalities_processed_str = "ALL_MUNS"
        # Ensure PARAMS access is safe with .get()
        processed_directive = self.sim.PARAMS.get('SPANISH_MUNICIPALITIES_TO_PROCESS')
        if isinstance(processed_directive, list):
            municipalities_processed_str = '_'.join(sorted(processed_directive))
        elif isinstance(processed_directive, str) and processed_directive.upper() == 'ALL':
            municipalities_processed_str = 'ALL_MUNS'
        else:
            municipalities_processed_str = 'UNKNOWN_MUNS'

        self.save_name = '{}/{}_states_{}_muns_{}'.format(
            AGENTS_PATH,
            '_'.join([str(self.sim.PARAMS.get(name, 'PARAM_NOT_FOUND')) for name in GENERATOR_PARAMS]),
            '_'.join(sorted(list(sim.geo.states_on_process))), # Ensure consistent order
            municipalities_processed_str)
    ...
    def save_stats_report(self, sim, bank_taxes):
        ...
        # Use 'pie' consistent with changes in funds.py and params.py
        for k in ['equally', 'locally', 'pie']: 
            mun_applied_treasure[k] = sum(r.applied_treasure.get(k, 0) for r in sim.regions.values()) # Use .get for safety
        ...
        report = f"{sim.clock.days};{price_index:.3f};{gdp_index:.3f};{gdp_growth:.3f};{unemployment:.3f};" \
                 ...
                 f"{mun_applied_treasure.get('pie', 0):.4f};{mun_applied_treasure['bank']:.4f}\n" # Use .get for 'pie'
    ...
    def save_regional_report(self, sim):
        ...
        for mun_id, regions in municipalities.items():
            ...
            mun_applied_treasure = defaultdict(int)
            # Use 'pie' consistent with changes
            for k in ['equally', 'locally', 'pie']:
                mun_applied_treasure[k] = sum(r.applied_treasure.get(k, 0) for r in regions) # Use .get for safety
            ...
            reports.append('%s;%s;%.3f;%d;%.3f;%.4f;%.3f;%.4f;%.5f;%.3f;%.6f;%.6f;%.6f;%.6f;%s'
                           % (sim.clock.days, mun_id, commuting, mun_pop, mun_gdp, mun_gini, mun_house_values,
                              ...
                              mun_applied_treasure.get('pie', 0), # Use .get for 'pie'
                              licenses))
